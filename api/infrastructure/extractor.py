...
class GVisionDocumentTextExtractor:
    ...
    def detect_document(self, gcs_source_uri, gcs_destination_uri):
    ...
    def upload_PDF_to_bucket(self, lease_id, document_path):
        ...
        # Get location of current file
        dirname = os.path.dirname(__file__)
        credentials_file_location = os.path.join(dirname, self.credentials_relative_file_location)

        storage_client = storage.Client.from_service_account_json(credentials_file_location)
    ...
    def extract_pdf_into_paragraphs(self, lease_id, document_path):
